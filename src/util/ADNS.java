...
/**
 * A class to read the data from the ADNS Laser tracking module with SPI library
 * SPI chip select 0
 * Copied from arduino sketch and translated into java
 * @author NeilHazra
 */
public class ADNS {
	DigitalInput ADNS_Interrupt;
	DigitalOutput ncs;
	SPI spi;
	...
	public ADNS() {
		spi = new SPI(SPI.Port.kOnboardCS0); //start SPI with CS0   
		ncs = new DigitalOutput(RobotMap.NCS); 
		ADNS_Interrupt = new DigitalInput(RobotMap.ADNSInturrupt);
		ADNSHandler ADNS_InterruptHandler  = new ADNSHandler();
		ADNS_Interrupt.requestInterrupts(ADNS_InterruptHandler);
		//ADNS_Interrupt.enableInterrupts();
		
		spi.setMSBFirst(); //Most significant bit first
		spi.setChipSelectActiveHigh(); // I think this is SPI mode 3
		spi.setClockActiveHigh(); // not sure
		
		adns_com_end(); // ensure that the serial port is reset 
		adns_com_begin(); // ensure that the serial port is reset
		adns_com_end(); // ensure that the serial port is reset
		adns_write_reg((byte) 0x3a, (byte) 0x5a); // force reset
		Timer.delay(0.0005); // wait for it to reboot
		adns_read_reg((byte)0x02);
		adns_read_reg((byte)0x03);
		adns_read_reg((byte)0x04);
		adns_read_reg((byte)0x05);
		adns_read_reg((byte)0x06);
		adns_upload_firmware();
		Timer.delay(0.0001);

	byte laser_ctrl0 = adns_read_reg((byte)0x20);
		adns_write_reg((byte)0x20, (byte) (laser_ctrl0 & 0xf0) );
	}
	//http://datasheet.octopart.com/ADNS-9800-Avago-datasheet-10666463.pdf
	void adns_upload_firmware(){
		  adns_write_reg((byte) 0x39, (byte) 0x02); 
		  adns_write_reg((byte) 0x13, (byte) 0x1d); 
		  Timer.delay(0.0001); 
		  adns_write_reg((byte) 0x13, (byte) 0x18); 
		  adns_com_begin();
		  byte[] dataToSend = new byte[1];
		  byte[] dataReceived = new byte[1];
		  dataToSend[0] = (byte) (0x62 | 0x80);
		  spi.transaction(dataToSend, dataReceived, 1);
		  Timer.delay(0.0001);
		  char c;
		  byte[] temp = new byte[1];
		  for(int i = 0; i < 3070; i++){ 
		   c = firmware_data[i];
		   temp[0] = (byte) c; //im not actually sure this works
		   spi.write(temp, 1); //this might be a transaction
		    Timer.delay(0.00015);
		  }
		  adns_com_end();
	}
	...
	byte adns_read_reg(byte reg_addr) {
		adns_com_begin(); // start
		byte[] dataReceived = new byte[1];
		byte[] dataToSend = new byte[1];
		dataToSend[0] = (byte) (reg_addr & 0x7f); 
		spi.transaction(dataToSend, dataReceived, 1); 
		Timer.delay(0.001);
		byte temp[] = new byte[1];
		temp[0] = 0;
		spi.transaction(temp, dataReceived, 1);
		Timer.delay(0.001);
		adns_com_end();
		Timer.delay(0.001);
	...
	void adns_write_reg(byte reg_addr, byte data) {
		...
		byte[] temp = new byte[1];
		byte[] dataToSend = new byte[1];
		dataToSend[0] = (byte) (reg_addr | 0x80);
		spi.transaction(dataToSend, temp, 1);
		// sent data
		dataToSend[0] = data;
		spi.transaction(dataToSend, temp, 1);
		Timer.delay(0.001);
	...
	public void adns_com_begin() {
		ncs.set(false);
	}

	public void adns_com_end() {
		ncs.set(true);
	}
	...
	public class ADNSHandler extends InterruptHandlerFunction {
		...
		public void interruptFired(int interruptAssertedMask, Object param) {
			ncs.set(false);
			xd = convTwosComp((int) adns_read_reg((byte) 0x03));
			adns_read_reg((byte) 0x04);
			yd = convTwosComp((int) adns_read_reg((byte) 0x05));
			adns_read_reg((byte) 0x06);
			x_total += xd;
			y_total += yd;

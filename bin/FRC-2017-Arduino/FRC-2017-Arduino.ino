...
void UpdatePointer(void){
  if(initComplete==9){
    ...
    totalX += float(*x)/200*2.54;
    totalY += float(*y)/200*2.54;
...
int convTwosComp(int b){
  ...
  void loop() {    
    ...
    Serial.print(totalY);  
    Serial.print(" ");
    //Serial.println(sqrt(totalX*totalX+totalY*totalY));
    Serial.println(atan2((totalY-prevY)/(millis()-prevMillis),(totalX- prevX)/(millis()-prevMillis))*180/PI);  
    ...
    xunion.myDouble = totalX;
    yunion.myDouble = totalY;
    thetaunion.myDouble = atan2((totalY-prevY)/(millis()-prevMillis),(totalX- prevX)/(millis()-prevMillis))*180/PI;
    
    
    
    for(int k = 0; k<4; k++)  {
      buffer[k]=xunion.myChars[k];
     //Serial.println(buffer[k]);  
  }
  
    for(int k = 4; k<8; k++)  {
      buffer[k]=yunion.myChars[k-4];
      //Serial.println(buffer[k]);
    }
    
    
    for(int k = 8; k<12; k++)  {
      buffer[k]=thetaunion.myChars[k-8];
    }
